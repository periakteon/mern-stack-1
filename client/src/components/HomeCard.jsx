  ...
  return (
    <div className='relative w-1/4 border p-3 rounded-md bg-gray-50 mx-5'>
      ...
      <div className='absolute -top-3 -right-3 flex items-center space-x-3'>
        <AiOutlineDelete onClick={() => deletePostFunction(post._id)} size={22} className='bg-red-500 rounded-full text-white p-1 cursor-pointer'/>
        <GrUpdate onClick={() => updatePostFunction(post._id)} size={22} className='bg-red-500 rounded-full text-white p-1 cursor-pointer'/>
      </div>
    </div>
  )
}
